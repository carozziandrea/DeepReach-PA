...
class CrawlerRunner:
    ...
    def __init__(self, config: OrchestratorConfig = None, crawler_dir: Path = None):
        ...
        if not self.crawler_dir.exists():
            print(f"⚠️  ATTENZIONE: Directory crawler non trovata: {self.crawler_dir}")
        elif not (self.crawler_dir / "scrapy.cfg").exists():
            print(f"⚠️  ATTENZIONE: scrapy.cfg non trovato in: {self.crawler_dir}")
        else:
            print(f"📂 Crawler directory: {self.crawler_dir}")

        # Directory dove il spider salva l'output (pa_crawler/output/)
        self.spider_output_dir = self.crawler_dir / "pa_crawler" / "output"
        ...
        for output_name in output_names:
            ...
            for file_path in paths_to_clean:
                if file_path.exists():
                    try:
                        os.remove(file_path)
                        print(f"🗑️  Rimosso file esistente: {file_path}")
                    except OSError as e:
    ...
    def _run_scrapy(
        ...
        print(f"\n   ▶️  Esecuzione: scrapy crawl {self.config.crawl.spider_name}")
        print(f"   📂 Directory: {cwd_path}")
        print("-" * 50)
    ...
    def run_shallow(self, homepage_url: str) -> CrawlInfo:
        """
        ...
        if success:
            ...
            print(f"   ✅ Completato in {duration:.1f}s - {info.total_pages} pagine")
        else:
            print(f"   ❌ Fallito: {error}")
        
        return info
    
    def run_deep(self, at_url: str) -> CrawlInfo:
        """
...
if __name__ == "__main__":
    ...
    print(f"\nRisultati:")
    print(f"  Shallow: {'✅' if shallow.success else '❌'} - {shallow.total_pages} pagine")
    print(f"  Deep: {'✅' if deep.success else '❌'} - {deep.total_pages} pagine")